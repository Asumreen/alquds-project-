...
class User extends Model
{
    ...
    public function login($myusername = "", $mypassword = "")
    {
        ...
        if (preg_match(Logins, $myusername)) {
            ...
            //if $count is not 1 then ther is no user in the database have this login id or the password don't match this user
                ...
                // redirect("login");
                // redirect("login_regist.login");
                view('login_regist/login',['form_inputs'],['login_error'=>'يرجى التأكد من البيانات المدخلة ']  );
    ...
    public function registration($post)
    {
        ...
        if (mysqli_num_rows($res) > 0) {
            ...
            view('login_regist/signup',['form_inputs'=>$post],['reg_error'=>'لقد تم تقديم طلب بأستخدام الرقم الوطني المدخل مسبقاً']  );
            ...
            if ($res) {
                if (isset($_SESSION['form_inputs']))
                    ...
                    view('login_regist/signup',['res'=>1] );
                    // redirect("registration", 1, "res");
                
            } else
                // Error('reg_error', "Error");
                view('login_regist/signup',['form_inputs'=>$post] ,['reg_error'=>'Error'] );
    ...
    public function get_all_user_info($ID, $view)
    {
        $db = $this->ConnectToDataBase();
        ...
        $info['Unit'] = $row['Unit'];
        $_SESSION['unit'] =  $info['Unit'];
        $info['Name'] = $row['Name'];
        $info['Address'] = $row['Address'];
        $info['Super'] = $row['Super'];
        $info['JoinDate'] = $row['JoinDate'];
        $info['BarthDate'] = $row['BarthDate'];
        $info['Relatev'] = $row['Relatev'];
        $info['Phone'] = $row['Phone'];
        $info['Email'] = $row['Email'];

        redirect("role/" . $view, $info, 'user_data');
    }
